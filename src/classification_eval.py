...
def run_inference_with_loss(model, loader, device):
    ...
    return all_labels, all_preds, all_logits, all_probs, avg_loss
...
def main():
    ...
    val_y, val_pred, val_logits, val_probs, val_loss = run_inference_with_loss(model, val_loader, device)
    test_y, test_pred, test_logits, test_probs, test_loss = run_inference_with_loss(model, test_loader, device)
    ...
    np.save(save_dir / f"{stem}_y_true.npy", np.array(test_y, dtype=np.int64))
    np.save(save_dir / f"{stem}_y_pred_text.npy", np.array(test_pred, dtype=np.int64))
    np.save(save_dir / f"{stem}_y_logits_text.npy", np.array(test_logits, dtype=np.float32))
    np.save(save_dir / f"{stem}_y_probs_text.npy", np.array(test_probs, dtype=np.float32))

    np.save(save_dir / f"{stem}_val_y_true.npy", np.array(val_y, dtype=np.int64))
    np.save(save_dir / f"{stem}_val_y_pred_text.npy", np.array(val_pred, dtype=np.int64))
    np.save(save_dir / f"{stem}_val_y_logits_text.npy", np.array(val_logits, dtype=np.float32))
    np.save(save_dir / f"{stem}_val_y_probs_text.npy", np.array(val_probs, dtype=np.float32))
    # 🔥 SAVE METRICS + ARTIFACTS (ADD THIS BLOCK)
        ...
        json.dump(
            {
                ...
                "artifacts": {
                    "test_y_true": f"{stem}_y_true.npy",
                    "test_y_pred": f"{stem}_y_pred_text.npy",
                    "test_y_logits": f"{stem}_y_logits_text.npy",
                    "test_y_probs": f"{stem}_y_probs_text.npy",
    
                    "val_y_true": f"{stem}_val_y_true.npy",
                    "val_y_pred": f"{stem}_val_y_pred_text.npy",
                    "val_y_logits": f"{stem}_val_y_logits_text.npy",
                    "val_y_probs": f"{stem}_val_y_probs_text.npy",
                }
            },
            f,
            indent=2,
