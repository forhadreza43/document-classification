...
def main():
    ...
    val_labels, val_preds, val_logits, val_probs, val_loss = run_inference_with_loss(model, val_loader, device)
    ...
    test_labels, test_preds, test_logits, test_probs, test_loss = run_inference_with_loss(model, test_loader, device)
    ...
    np.save(save_dir / f"{stem}_y_true.npy", np.array(test_labels, dtype=np.int64))
    np.save(save_dir / f"{stem}_y_pred_multi.npy", np.array(test_preds, dtype=np.int64))
    np.save(save_dir / f"{stem}_y_logits_multi.npy", np.array(test_logits, dtype=np.float32))
    np.save(save_dir / f"{stem}_y_probs_multi.npy", np.array(test_probs, dtype=np.float32))

    np.save(save_dir / f"{stem}_val_y_true.npy", np.array(val_labels, dtype=np.int64))
    np.save(save_dir / f"{stem}_val_y_pred_multi.npy", np.array(val_preds, dtype=np.int64))
    np.save(save_dir / f"{stem}_val_y_logits_multi.npy", np.array(val_logits, dtype=np.float32))
    np.save(save_dir / f"{stem}_val_y_probs_multi.npy", np.array(val_probs, dtype=np.float32))
        ...
        json.dump(
            {
                ...
                "artifacts": {
                    ...
                    "test_y_pred": f"{stem}_y_pred_multi.npy",
                    "test_y_logits": f"{stem}_y_logits_multi.npy",
                    "test_y_probs": f"{stem}_y_probs_multi.npy",

                    "val_y_true": f"{stem}_val_y_true.npy",
                    "val_y_pred": f"{stem}_val_y_pred_multi.npy",
                    "val_y_logits": f"{stem}_val_y_logits_multi.npy",
                    "val_y_probs": f"{stem}_val_y_probs_multi.npy",
                }
            },
            f,
            indent=2,
