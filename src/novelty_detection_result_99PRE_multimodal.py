...
def main():
    ...
    ap.add_argument("--ckpt", type=str, required=True, help="Path to a single checkpoint .pt file.")
    ap.add_argument(
        "--use",
        type=str,
        default=None,
    ...
    ap.add_argument("--save_dir", type=str, default="eval_outputs_99pre_multimodal")
    args = ap.parse_args()
    ...
    out_path = save_dir / f"{ckpt_path.stem}_pre99_metrics.json"
    with out_path.open("w", encoding="utf-8") as f:
        json.dump(
            {
                "checkpoint": str(ckpt_path),
                "target_pre": args.target_pre,
                "train_samples": cfg.train_samples,
                "val_samples": cfg.val_samples,
                "test_samples": cfg.test_samples,
                "knn": metrics["knn"],
                "knn_star": metrics["knn_star"],
            },
            f,
            indent=2,
        )
